...
(function($){
	...
	view = {
		...
		makeDOMNodeNameBefore: function(html, setting, node) {
		    ...
		    /*整合dwz*/
		    if (node.external) {
		       html.push("external='", node.external,"'");
		    }
		...
		setNodeRel: function(setting, node) {//整合dwz
		    var aObj = $$(node, consts.id.A, setting);
		     var rel = node.rel;
		...
		setNodeExternal: function(setting,node){//整合dwz
			var aObj = $$(node, consts.id.A, setting);
		     var external = node.external;
		    if (external == null || external.length == 0) {
		      aObj.removeAttr("external");
		    } else {
		      aObj.attr("external", external);
		    }
		},
			...
			var zTreeTools = {
				...
				updateNode : function(node, checkTypeFlag) {
					...
					if (nObj.get(0) && tools.uCanDo(setting)) {
						...
						view.setNodeRel(setting, node);//整合dwz
						view.setNodeExternal(setting,node);//整合dwz
					}
				}
			}
